...
public:
  ...
  static void initial(real_type time, const shared_state &shared,
    ...
    const double rt = shared.beta * p_susc * 1.0 / shared.gamma;
  ...
  static void rhs(real_type time, const real_type *state,
    ...
    const real_type rt = shared.beta * S * 7.0 / shared.N;
  ...
  static auto events(const shared_state &shared,
    ...
    auto fn_state_test_off = [&](const double t, const double *y) {
      const double diff = y[5] - 1.0; // rt < 1.0
      Rprintf("time = %f, rt-1.0 = %f \n", t, diff);
    ...
    // save ipr to ipr_saved, refer to absolute positions in y
    ...
    dust2::ode::event<real_type> event_state_off(
        name_state_off, {5}, fn_state_test_off, fn_dummy_action);
